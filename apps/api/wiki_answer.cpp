                         ...
                         std::string                         org_unit_id)
{
    ...
    try {
        ...
        if (body.system_prompt)   // caller override — empty is allowed and
                                  // means "no system message"; the answer
                                  // path may still work for evaluation.
            opts.context_opts.system_prompt = *body.system_prompt;
