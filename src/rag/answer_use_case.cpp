                   ...
                   const AnswerOptions&   opts) const
{
    ...
    // 2. Build the prompt. Apply the system-prompt override with
    //    three-state semantics (nullopt = default, present = verbatim
    //    including empty). See AnswerOptions::system_prompt_override.
    ContextBuilderOptions builder_opts = opts.context_opts;
    if (opts.system_prompt_override.has_value())
        builder_opts.system_prompt = *opts.system_prompt_override;
    else
    ...
    // Number of caller-supplied evidence items that reached the
    // prompt, sourced directly from ContextBuilder's counter (not
    // computed from citation-ID list sizes — those count synthesized
    // endpoint chunks separately, which is a different question).
    out.evidence_included = prompt->evidence_included;
