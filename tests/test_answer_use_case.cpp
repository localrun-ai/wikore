          ...
          "[integration][answer]")
{
    ...
    wikore::rag::AnswerOptions opts;
    opts.intent = wikore::rag::RetrievalIntent::Fact;
    opts.system_prompt_override = "You are a strict evaluator harness.";
...
TEST_CASE("AnswerUseCase caller-supplied EMPTY system prompt is honoured, not defaulted",
          "[integration][answer]")
{
    // P3 contract from #61 review: an eval harness that sets
    // system_prompt_override to an empty string wants a bare prompt.
    // Previously an empty string silently got the default; now it
    // reaches the LLM verbatim as a zero-length system message.
    if (!db_available()) SKIP("DATABASE_URL not set");
    auto db = wikore::Db::get();
    auto f  = seed(db);
    auto llm = std::make_shared<StubLlm>();
    ...
    wikore::rag::AnswerOptions opts;
    opts.intent = wikore::rag::RetrievalIntent::Fact;
    opts.system_prompt_override = std::string{};
    ...
    REQUIRE(cap.messages.size() == 2);
    CHECK(cap.messages[0].role    == "system");
    CHECK(cap.messages[0].content.empty());
    CHECK(cap.messages[0].content.find("origin=llm_proposal") == std::string::npos);
}

TEST_CASE("AnswerUseCase evidence_included counts caller-supplied items, not synth SRCs",
          "[integration][answer]")
{
    // P4 contract from #61 review: evidence_included is the number of
    // AllowedEvidence items that reached the prompt (via
    // ContextBuilder), NOT the citation-ID list sizes. A single
    // Bridge relationship contributes 1 evidence item + 2 synthesized
    // endpoint SRCs + 1 REL citation — evidence_included must be 1,
    // not 3.
    if (!db_available()) SKIP("DATABASE_URL not set");
    auto db = wikore::Db::get();
    auto f  = seed(db);
    ...
    auto r = drogon::sync_wait(uc->run(make_ctx(f.user), "q", f.team, opts));
    REQUIRE(r);
    // Sanity: this bridge query returns exactly one relationship.
    CHECK(r->source_edge_ids.size()  == 1);
    CHECK(r->source_chunk_ids.size() == 2);   // synthesized endpoint SRCs
    // Evidence-included counter is the caller-supplied item count.
    CHECK(r->evidence_included == 1);
}
